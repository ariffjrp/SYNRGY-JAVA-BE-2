...
public class Tampilan {
    ...
    public void cetakPesanan(ArrayList<String> pesanan, ArrayList<Integer> hargaMakanan, ArrayList<Integer> qtyMakanan, int totalHarga) {
        if (!pesanan.isEmpty()) {
            ...
            IntStream.range(0, pesanan.size()).forEach(i -> {
                ...
                if (!catatanTambahan.isEmpty()) {
                    namaMakanan += " (" + catatanTambahan + ")";
                }

                String formattedTotalHargaItem = formatHarga(totalHargaItem);
                System.out.println(namaMakanan + " (x" + qty + ")" + Konstan.TAB + formattedTotalHargaItem);
            });
